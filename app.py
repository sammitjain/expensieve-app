                ...
                messages=[
                    {
                        ...
                        "content": [
                            { "type": "text", "text": """"
                                                            ...
                                                            Given an image of a receipt, always return a JSON object with the following fields in this order: Date, Expense Type, Vendor, City, Currency, and Amount. 
                                                            If available, also extract Payment Mode and Time of Transaction. In most cases receipts are from within India - so you can guess INR from the city.
                                                            The date should be always be DD/MM/YYYY format (Indian style) and typically in Indian receipts the dates will be mentioned in a similar manner. 
                                                            Restrict Expense Type to one of the following categories:
                                                            1. Cab/Ground Transportation
                                                            2. Meal
                                                            3. Hotel
                                                            4. Telecom (for phone bills, etc.)
                                                            5. Other (if yes, then mention something in remarks)

                                                            If any field is missing or unclear, leave it as null and mention it in the 'Remarks' field. Do not guess missing values. Format the output as JSON. The response should contain only the JSON output and no extra words.
                                                    """ },
                            {
                                "type": "image_url",
                                "image_url": {
